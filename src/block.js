...
export const blockAction = (instance, engine, time) => {
  ...
  switch (i.status) {
    ...
    case constant.rotateRight:
      ...
      const ROTATE_SLOW = 0.00003;
      const rotateSpeed = engine.pixelsPerFrame(Math.PI * 4) * ROTATE_SLOW;

      const isShouldFall = isRight ? instance.rotate > 5 : instance.rotate < -5;
      const leftFix = isRight ? 1 : -1;

      if (isShouldFall) {
        instance.rotate += (rotateSpeed / 12) * leftFix;
        ...
        rotateRatio = Math.max(rotateRatio, 0.5);
